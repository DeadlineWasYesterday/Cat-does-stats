...
g <- seq(1:1000)
#simulate genomes
g1 <- c(rbind(g, rep('-', length.out = 1000)))
g2 <- c(rbind(g, rep('-', length.out = 1000)))
#remove some genes randomly
g1[as.integer(runif(100,1,500))*2 -1] = 0
g2[as.integer(runif(100,1,500))*2 -1] = 0
#calculate differences
dif = sum(g1!=g2)
#use those differences in shamelessly copied code
#152 out of 1000

##Shamelessly copied from https://dosreislab.github.io/2016/05/17/write-your-own-bayesian-program.html
...
L <- function(d, x, n) (1/2 - 1*exp(-2*d/1)/2)^x * (1/2 + 1*exp(-2*d/1)/2)^(n - x)

# plot the likelihood:
curve(L(x, 152, 1000), from=0, to=1, n=10000, xlab="distance, d", ylab="likelihood, p(x | d)")

# The maximum likelihood estimate is (the highest point in the curve)
d.mle <- -1/2 * log(1 - 2/1 * 152/1000) # [1] 0.1812028
abline(v=d.mle, lty=2)
title(expression(hat(d) == 0.1812028))
...
integrate(d.upost, lower=0, upper=Inf, x=152, n=1000, mu=0.2, abs.tol=0)
# 6.849909e-187 with absolute error < 1.9e-135
C  <- 1 / 6.849909e-187

# The posterior distribution function (when x=90, n=948 and mu=0.2)is thus
d.post <- function(d) C * d.upost(d=d, x=152, n=1000, mu=0.2)

# Plot posterior distribution:
curve(d.post(x), from=0, to=0.5, n=500, xlab="d", ylab="density")
# Add prior:
...
# 0.1816672 with absolute error < 9.4e-06
# Add it to the plot:
abline(v=0.1816672, lty=3) # posterior mean
...
# 1 with absolute error < 1.6e-08, i.e. 56.3%
...
d.mcmcf <- function(init.d, N, w) {
  ...
  for (i in 1:N) {
    ...
    p.ratio <- d.upost(d.prop, x=152, n=1000, mu=0.2) /
      d.upost(d[i], x=152, n=1000, mu=0.2)
...
# Add 95% CI as reference:0.1532103, 0.2173143
abline(h=c(0.1532103, 0.2173143), lty=2, col="red")
...
mean(d.l) # should be close to 0.1815297
...
abline(h=c(0.1532103, 0.2173143), lty=2)
...
rt.upost <- function(r, t, x=152, n=1000) {
